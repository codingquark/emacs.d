...
** Tools
*** Web Search
Register a ~gptel~ tool that queries the web via [[https://serper.dev][Serper.dev]] (Google
search API).  The tool is named ~web_search~ and is offered to any
model that supports function calling through the OpenRouter backend.

The Serper API key is read from =auth-source= (e.g. =~/.authinfo=)
under the host ~serper.dev~.  Example entry:

#+BEGIN_EXAMPLE
machine serper.dev login apikey password YOUR_KEY_HERE
#+END_EXAMPLE
