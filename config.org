...
~eglot~ is Emacs' built-in language-server frontend, shared by Lua and
Rust: each language mode starts its own server (lua-language-server and
rust-analyzer, respectively) when a file is opened.
