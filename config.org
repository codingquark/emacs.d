...
[[https://github.com/steveyegge/efrit][Efrit]] is Steve Yegge's AI coding assistant for Emacs — a
Claude-powered agent that can execute natural language commands,
explore codebases, and run shell commands. Requires an Anthropic API
key (see `M-x efrit-doctor` for setup). Zero client-side intelligence:
...
** Web Search
Web search is available via OpenRouter's ~openrouter:web_search~ server
tool, exposed as a normal ~gptel-tools~ entry named
~openrouter_web_search~. It is disabled by default, so routine chats and
commit-message generation cannot spend search credits by accident. Toggle
it with ~M-x gptel-tools~ or use ~M-x cq-gptel-toggle-web-search~ for a
quick global toggle.

When selected, the tool is translated into OpenRouter's server-tool JSON,
not a local Emacs function call. Each request lets the model decide whether
to search. The tool is capped at three results total per request and uses
low search context to keep OpenRouter search costs bounded.
...
Use the host's existing mbsync and notmuch setup. Mail lives under =~/Mail=,
with the notmuch database and Maildirs already managed outside
Emacs. The Homebrew notmuch package installs its Emacs Lisp outside the
default app bundle load path, so add the stable opt paths before registering
the package. Personal names, addresses, identities, and folder names live in
=private/email.el=, which is ignored by git.

Example local file:

#+BEGIN_EXAMPLE
(setq cq-email-full-name "Your Name"
      cq-email-address "you@example.com"
      cq-notmuch-fcc-dirs '(("you@example\\.com" . "account/Sent +sent -unread -inbox")
                            (".*" . "default/Sent +sent -unread -inbox"))
      cq-notmuch-draft-folder "default/Drafts"
      cq-notmuch-identities '("Your Name <you@example.com>"))
#+END_EXAMPLE
