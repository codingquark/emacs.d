...
Use Modus Vivendi by default and keep the usual manual toggle on
regular hosts. Emacs Plus follows macOS appearance changes without polling.
On Omarchy, select Modus Operandi or Modus Vivendi from the desktop's
light or dark appearance through the integration below.
...
Configure the core font faces: ~cq-font-family~ and ~cq-font-height~ drive the
monospace font used by code and tabular buffers, Charter covers variable-pitch
reading buffers such as `shr` renderers. A face is only touched when its font
is actually installed, so a host missing one font keeps the previous frame font.

Hosts that want something different create =~/.config/emacs/private/fonts.el=
(gitignored) with plain ~setq~ forms. It is loaded after the defaults below and
before any face is applied, so its values win over the shared defaults:

#+BEGIN_EXAMPLE
(setq cq-font-family "IBM Plex Mono"
      cq-font-height 150)
#+END_EXAMPLE
...
Respect directory-specific formatting rules from ~.editorconfig~ files.
...
Enable built-in Hideshow for collapsing JSON objects and arrays.
...
Use built-in Flyspell with Aspell for spell checking in prose-oriented modes
derived from ~text-mode~, including plain text, Org, and Markdown.
...
Configure dired with denote integration and file-type icons.
...
Set up Magit for repository status and everyday Git workflows. Enable
Delta highlighting only on hosts with the =delta= executable
installed.
...
Connect with ~C-c i~ (~cq-erc-libera~).
...
HEY ships colour-free faces, so the header line takes its Modus colours from
~cq-hey-style-modus-header-faces~: the source name uses the same palette entry
Modus gives Elfeed's feed face, and the refresh time takes the date accent.
Specs go on the enabled Modus theme rather than the ~user~ theme, so they
retire with it and leave no stale colours behind under a non-Modus theme.
The hook that ~modus-themes-load-theme~ runs re-derives
them on startup, on ~<f5>~, and on an Omarchy appearance flip; the direct call
covers the theme this session has already loaded.
...
Use the built-in Unix configuration mode for MySQL-style =.cnf= files.
...
it last and override its theme application to use Modus. Advice survives the
shim reloads performed by Omarchy's theme and font hooks. Use Omarchy's color
resolver for light/dark detection, with the shim's predicate as a fallback on
older installations. The shim is absent on regular hosts.
